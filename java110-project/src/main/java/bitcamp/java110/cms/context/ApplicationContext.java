...
public class ApplicationContext {
    ...
    public ApplicationContext(String packageName) throws Exception {
        ...
        // 패키지 폴더에 들어 있는 클래스를 찾아 클래스 정보를 로딩한 후 그 목록에 저장한다.
        findClass(file, path);


        //로딩된 클래스 목록을 뒤져서 @Component가 붙은 클래스에 대해서
        // 인스턴스를 생성하여 objPool에 보관한다.
        createInstance();

        //objPool에 보관된 객체를 꺼내 @Autowired에 붙은 setter를 찾아 호출한다.
        // ==> 의존 객체 주입
        injectDependency();
    }
    ...
    //객체의 타입으로 objPool 에 보관된 객체를 찾아 리턴한다.
    ...
    private void createInstance() {
        for(Class<?> clazz : classes) {
            ...
            // @Component 애노테이션이 붙지 않는 클래스는 객체를 생성하지 않는다
    ...
    private void injectDependency() {
        ...
        for(Object obj : objList){
            //목록에서 객체를 꺼내 @Autowired 가 붙은 메서드를 찾는다.
            ...
            for(Method m : methods) {
                ...
                //setter 메서드의 parameter 타입을 알아낸다.
