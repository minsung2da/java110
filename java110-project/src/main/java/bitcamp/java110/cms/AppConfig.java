...
public class AppConfig {
    ...
    // 트랜잭션 관리자의 이름은 반드시 "transactionManager"야 한다.
    // 그래서 메서드 이름을 다음과 같이 지은 것이다.
    // Spring 에서 트랜잭션 관리자를 찾을 떄 이 이름으로 찾는다.
    ...
    public PlatformTransactionManager transactionManager(DataSource dataSource) {
       ...
       //트랜잭션 관리자가 하는 일은 DB 커넥션에 commit과 rollback을 다루는 것이다.
       //따라서 트랜잭션 관리자는 DB 커넥션을 제공해주는
       //DataSource(DB 커넥션 풀)가 필요하다
       // 그래서 트랜잭션 관리자를 만들 때 반드시 DataSource 객체를 넘겨줘야한다.
       // 물론 관리자 객체를 만든 후에 세터를 호출해서 넘겨줘야 한다.
