...
//   XML 설정파일에 지정된 문자열을 해당 파라미터 타입에 맞게 자동으로 형변환한다.
//   단 primitive 타입으로만 자동 형변환 한다.
//
//   예) String -> byte/short/int/long/float/double/char/boolean
//
...
//   - 구현할 메서드가 너무 많기 때문에 보통 이 인터페이스를 직접 구현하기 보다
//     이 인터페이스를 미리 구현한 클래스를 상속 받아 만든다.
// =>  java.beans.PropertyEditorSupport 클래스를 상속 받는다.
//   - 이 클래스를 상속받아서 setAsText(String) 메서드를 오버라이딩 한다.
//   - setAsText() 메서드는 파아미터 값으로 String을 받아서
